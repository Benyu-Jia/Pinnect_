...
const MomentsScreen = React.memo(() => {
...
const Moments = React.memo(
  (props: {
    ...
    const loadPins = useCallback(async (oldData: string[], page: number) => {
      const result = await fetchAPI("recommend", "POST", {
    ...
    if (pins.length == 0) {
      return (
        <View style={styles.noMomentsContainer}>
          <FontistoIcon name="wink" size={36} style={styles.noMomentsText} />
          <AppText style={styles.noMomentsText}>Running...</AppText>
        </View>
      );
    }
